...
namespace Common.Utility
{
    ...
    public class MemoryCaches<T>
    {
        ...
        /// </summary>
        public MemoryCaches()
            : this(Guid.NewGuid().ToString(), TimeSpan.FromHours(1))
        {
        }
        ...
        {
            var values = this.memoryCaches.ToArray();
            foreach (var item in values)
            {
                var value = item.Value as CacheValue;
                if (value != null)
                {
        ...
        public T GetOrAdd(string key, T mode)
        {
            using (asyncRoot.Lock())
            {
                ...
                if (value == null)
                {
                    value = new CacheValue { Value = mode };
