...
namespace Redis
{
    ...
    public class RedisHelper
    {
        ...
        /// <param name="DefaultKey">默认key，默认是 Default_ </param>
        /// <param name="db">db库</param>
        public RedisHelper(string redisConnectionString, string DefaultKey = "Default_", RedisDb db = RedisDb.Db0)
        {
            ConnectionString = redisConnectionString;
            _connMultiplexer = ConnectionMultiplexer.Connect(ConnectionString);
            _DefaultKey = DefaultKey;
            _db = _connMultiplexer.GetDatabase(db.GetHashCode());
        }
        #endregion 构造函数
